...
public class JavadocsTest {
	private static final String JAVADOCS_PATH = System
			.getProperty("javadocDir");
	private static final String RESOURCES_PATH = "src/test/resources";
	private static final Logger LOGGER = Logger.getLogger(JavadocsTest.class
			.getName());
	...
	public static Collection<Object[]> getFiles() {
		...
		for (final File f : new File(JAVADOCS_PATH).listFiles()) {
			if (f.isDirectory()) {
	 ...
	 * Constructor.
	 *
	 * @param file Current working directory
	 */
	...
	public void compareJavadocs() {
		...
		// cycle
		final File[] filesList = workDir.listFiles();

		// For every .jar file in the current workDir
		for (int i = 0; i < filesList.length; i++) {
			// Decide the destination folder name
			final String destination = (i % 2 == 0) ? "first" : "second";
			// Get the .jar file
			final File f = filesList[i];
			// Unzip the .jar file
			final String tempPath = unzipFile(f, destination);
		...
		final File finalDiff = executePythonScript("removeMovedWithoutHeaders");
	 ...
	 * generated javadocs.
	 *
	 * @param folder folder with extracted javadoc
	 */
	public void removeDates(File folder) {
		try {
			final Runtime r = Runtime.getRuntime();
			final Process p = r.exec(new File(RESOURCES_PATH).getAbsolutePath()
	 ...
	 */
	public void doDiff() {
		final String diffCmd = "diff -ur -x *.MF -x *.LIST -x *.css -x *.gif "
	 ...
	 *
	 * @param scriptName name of the python script file
	 * @return file with the output of the python script
	 */
	public File executePythonScript(String scriptName) {
		final PythonInterpreter interp = new org.python.util.PythonInterpreter();
		FileInputStream fis = null;
		final File outputFile = new File(workDir + File.separator + scriptName
				+ ".diff");
	...
	public void removeUnnecessaryFiles() {
		for (final File f : workDir.listFiles()) {
			if (f.isDirectory()) {
				recursiveDelete(f);
			}
